...
func TestResultNotes(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "pending",
			res:  event.Result{Pending: true},
			want: "⏳",
		},
		{
			name: "nothing to do",
			res:  event.Result{Coverage: 80},
			want: "—",
		},
		{
			name: "commits without a coverage change",
			res:  event.Result{Commits: []string{"fmt", "vet"}, PrevCoverage: 80, Coverage: 80},
			want: "fmt, vet → pushed",
		},
		{
			name: "commits with a coverage gain",
			res:  event.Result{Commits: []string{"fmt"}, PrevCoverage: 80, Coverage: 85.5},
			want: "fmt → pushed (+5.5%)",
		},
		{
			name: "commits with a coverage loss",
			res:  event.Result{Commits: []string{"fmt"}, PrevCoverage: 80, Coverage: 75},
			want: "fmt → pushed (-5.0%)",
		},
		{
			name: "no delta without a previous figure to compare against",
			res:  event.Result{Commits: []string{"fmt"}, PrevCoverage: 0, Coverage: 42},
			want: "fmt → pushed",
		},
		{
			name: "error",
			res:  event.Result{Err: errors.New("boom")},
			want: "ERR: boom",
		},
		{
			name: "a long error is truncated to keep the table readable",
			res:  event.Result{Err: errors.New(strings.Repeat("x", 100))},
			want: "ERR: " + strings.Repeat("x", 60) + "…",
		},
		{
			name: "an error outranks the commits that preceded it",
			res:  event.Result{Commits: []string{"fmt"}, Err: errors.New("boom")},
			want: "ERR: boom",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			// notes is unexported, so go through the table it feeds.
			table := event.RenderTable([]event.Result{tc.res})
...
func TestResultErrorMessage(t *testing.T) {
	t.Run("no error yields no message", func(t *testing.T) {
		if got := (event.Result{}).ErrorMessage(); got != "" {
...
func TestBoard(t *testing.T) {
	...
	t.Run("every row starts pending", func(t *testing.T) {
		for i, res := range event.NewBoard(rows).Results() {
			if !res.Pending {
	...
	t.Run("results keep board order, not arrival order", func(t *testing.T) {
		b := event.NewBoard(rows)

		b.Set(event.Result{Owner: "o", Name: "third", Coverage: 30})
		b.Set(event.Result{Owner: "o", Name: "first", Coverage: 10})
	...
	t.Run("a result for an unknown repository is ignored", func(t *testing.T) {
		b := event.NewBoard(rows)
		b.Set(event.Result{Owner: "o", Name: "stranger"})
	...
	t.Run("concurrent updates are safe", func(t *testing.T) {
		...
		for _, row := range rows {
			wg.Go(func() {
				b.Set(event.Result{Owner: row.Owner, Name: row.Name, Coverage: 50})
				b.Render()
			})
		}
...
func TestKey(t *testing.T) {
	if got, want := event.Key("owner", "name"), "owner/name"; got != want {
		t.Errorf("got %q, want %q", got, want)
	}

	if got, want := (event.Result{Owner: "o", Name: "n"}).Key(), "o/n"; got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
...
func TestResultNotable(t *testing.T) {
	for _, tc := range []struct {
		...
		{"quiet run", event.Result{Coverage: 80}, false},
		{"pushed", event.Result{Pushed: true}, true},
		{"failed", event.Result{Err: errors.New("boom")}, true},
		...
		{"committed but not pushed", event.Result{Commits: []string{"fmt"}}, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
